...
public class MultiServiceModelingTest {
    ...
    private static final String MULTISERVICE_MODEL = "solutionArchitectureModelingService/getComponentModel";

    @Test(description = "Test simple model generation")
    public void testSimpleService() throws IOException, ExecutionException, InterruptedException {
        ...
        ComponentModelRequest request = new ComponentModelRequest();
        ...
        ComponentModelResponse response = (ComponentModelResponse) result.get();
        if (!Objects.equals(response, "")) {
            Assert.assertTrue(true);
        }
    }

    @Test(description = "Test complex model generation")
    public void testFunction() throws IOException, ExecutionException, InterruptedException {
        Path project = RES_DIR.resolve(BALLERINA).resolve(Path.of("reservation_api",  "reservation_service.bal").toString());
