...
import static io.ballerina.componentmodel.ComponentModelingConstants.CLIENT_ANNOTATION;
import static io.ballerina.componentmodel.ComponentModelingConstants.ID;
import static io.ballerina.componentmodel.ComponentModelingConstants.SERVICE_ANNOTATION;
import static io.ballerina.componentmodel.ComponentModelingConstants.SERVICE_ID;
...
public class VisitorUtil {
    public static String getId(NodeList<AnnotationNode> annotationNodes) {
        ...
        for (AnnotationNode annotationNode : annotationNodes)  {
            ...
            if (annotationName.equals(SERVICE_ANNOTATION) || annotationName.equals(CLIENT_ANNOTATION)) {
                    ...
                    fields.forEach(mappingFieldNode -> {
                        if (mappingFieldNode.kind() == SyntaxKind.SPECIFIC_FIELD) {
                            ...
                            if (specificFieldNode.valueExpr().isPresent()) {
                                if ((name.equals(ID) && annotationName.equals(SERVICE_ANNOTATION)) ||
                                        (name.equals(SERVICE_ID) && annotationName.equals(CLIENT_ANNOTATION))) {
