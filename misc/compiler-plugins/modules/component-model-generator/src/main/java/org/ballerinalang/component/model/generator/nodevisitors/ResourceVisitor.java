...
public class ResourceVisitor extends NodeVisitor {
    ...
    private String getParameterIn(NodeList<AnnotationNode> annotationNodes) {
        ...
        if (payloadAnnotation.isPresent()) {
            in = MultiServiceModelingConstants.ParameterIn.BODY.getValue();
        } else if (headerAnnotation.isPresent()) {
            in = MultiServiceModelingConstants.ParameterIn.HEADER.getValue();
        } else {
            in = MultiServiceModelingConstants.ParameterIn.QUERY.getValue();
        }
        return in;
    }
